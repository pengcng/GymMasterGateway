...
import { IPayment } from 'app/shared/model/bookingPaymentApp/payment.model';
...
export class PaymentService {
  public resourceUrl = SERVER_API_URL + 'services/bookingpaymentapp/api/payments';
  public resourceSearchUrl = SERVER_API_URL + 'services/bookingpaymentapp/api/_search/payments';
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      tranDt: payment.tranDt && payment.tranDt.isValid() ? payment.tranDt.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.tranDt = res.body.tranDt ? moment(res.body.tranDt) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.tranDt = payment.tranDt ? moment(payment.tranDt) : undefined;
      });
    }
    return res;
  }
